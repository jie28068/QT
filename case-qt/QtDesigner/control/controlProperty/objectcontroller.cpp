    ...
    for (int idx = metaObject->propertyOffset(); idx < metaObject->propertyCount(); idx++)
    {
        ...
        if (metaProperty.isReadable())
        {
            if (m_classToIndexToProperty.contains(metaObject) && m_classToIndexToProperty[metaObject].contains(idx))
            {
                ...
                {
                    auto name = subProperty->propertyName();
                    auto value = metaProperty.read(m_object);
                    auto aa = metaProperty.name();
...
void ObjectControllerPrivate::addClassProperties(const QMetaObject *metaObject)
{
    ...
    if (!classProperty)
    {
        ...
        classProperty = m_manager->addProperty(QtVariantPropertyManager::groupTypeId(), "属性"); // 组
        ...
        for (int idx = metaObject->propertyOffset(); idx < metaObject->propertyCount(); idx++)
        {
            ...
            {
                subProperty = m_manager->addProperty(QtVariantPropertyManager::enumTypeId(), GlobalDefinition::controlTrMap.value(metaProperty.name()).toString());
            ...
            else if (m_manager->isPropertyTypeSupported(type))
            {
                ...
                {
                    subProperty = m_manager->addProperty(type, GlobalDefinition::controlTrMap.value(metaProperty.name()).toString());
                }
                subProperty->setValue(metaProperty.read(m_object));
            }
            else
            {
                subProperty = m_readOnlyManager->addProperty(QVariant::String, (GlobalDefinition::controlTrMap.value(metaProperty.name())).toString());
