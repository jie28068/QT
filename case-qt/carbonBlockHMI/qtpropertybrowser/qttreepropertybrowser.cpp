...
QtPropertyEditorView::QtPropertyEditorView(QWidget *parent) : QTreeWidget(parent),
                                                              m_editorPrivate(0)
{
...
void QtPropertyEditorView::mousePressEvent(QMouseEvent *event)
{
    ...
    if (item)
    {
        if ((item != m_editorPrivate->editedItem()) && (event->button() == Qt::LeftButton) && (header()->logicalIndexAt(event->pos().x()) == 1) && ((item->flags() & (Qt::ItemIsEditable | Qt::ItemIsEnabled)) == (Qt::ItemIsEditable | Qt::ItemIsEnabled)))
        {
            editItem(item, 1);
        }
...
QtTreePropertyBrowserPrivate::QtTreePropertyBrowserPrivate() : m_treeWidget(0),
    ...
    : QtAbstractPropertyBrowser(parent), d_ptr(new QtTreePropertyBrowserPrivate)
{
    ...
    connect(d_ptr->m_treeWidget, SIGNAL(updateMenu()), this, SIGNAL(updateMenu()));
}
...
QModelIndexList QtTreePropertyBrowser::selectedIndexes() const
{
    return d_ptr->m_treeWidget->selectionModel()->selectedIndexes();
}
...
*/
void QtTreePropertyBrowser::setCustomMenu(QMenu *menu)
{
    d_ptr->m_treeWidget->m_menu = nullptr;
    d_ptr->m_treeWidget->m_menu = menu;
}
...
void QtTreePropertyBrowser::contextMenuEvent(QContextMenuEvent *event)
{
    d_ptr->m_treeWidget->m_menu->exec(event->globalPos());
}
