                         ...
                         int role)
{
  ...
  if (Qt::EditRole == role)
  {
    if (col == 1)
    {
      ...
      emit dataValueChanged(item->key(), value, oldValue);
      return true;
    }
  }
...
void QJsonModel::removeArrayItem(const QModelIndex &index)
{
  if (!index.isValid())
    return;

  QJsonTreeItem *item = static_cast<QJsonTreeItem *>(index.internalPointer());
  QJsonTreeItem *arrayItem = item->parent();
  if (!item || !arrayItem)
    return;
  QJsonTreeItem *parentItem = arrayItem->parent();

  if (!parentItem || parentItem->type() != QJsonValue::Array)
    return;

  beginRemoveRows(index.parent().parent(), arrayItem->row(), arrayItem->row());

  // 删除当前项
  parentItem->mChilds.removeAt(arrayItem->row());
  delete arrayItem;

  // 更新剩余项的下标
  for (int i = 0; i < parentItem->childCount(); ++i)
  {
    QJsonTreeItem *childItem = parentItem->child(i);
    childItem->setKey(QString::number(i));
  }

  endRemoveRows();
}

void QJsonModel::removeLastItem(const QModelIndex &index)
{
  if (!index.isValid())
    return;

  QJsonTreeItem *item = static_cast<QJsonTreeItem *>(index.internalPointer());
  auto lastitem = item->child(item->childCount() - 1);
  if (lastitem)
  {
    int lastRow = item->mChilds.indexOf(lastitem);
    QModelIndex parentIndex = index.parent();
    if (!parentIndex.isValid())
    {
      parentIndex = QModelIndex();
    }
    beginRemoveRows(index.parent(), lastRow, lastRow);
    item->mChilds.removeAt(lastRow);
    delete lastitem;
    endRemoveRows();
  }
}

int QJsonModel::getIndexChildCount(const QModelIndex &index)
{
  if (!index.isValid())
    return 0;
...
void QJsonModel::removeAllItems(const QModelIndex &index)
{
  if (!index.isValid())
    return;
  ...
  int endRow = startRow + item->childCount() - 1;

  beginRemoveRows(index.parent(), startRow, endRow);

  // 删除所有子项
  while (item->childCount() > 0)
  {
    QJsonTreeItem *childItem = item->child(0);
    item->mChilds.removeAt(0);
    delete childItem;
  }
...
void QJsonModel::setHearders(const QStringList &headers)
{
  mHeaders.clear();
...
bool QJsonModel::isTar(const QModelIndex &index)
{
  if (!index.isValid())
    return false;

  QJsonTreeItem *item = static_cast<QJsonTreeItem *>(index.internalPointer());
  auto itemlist = item->mChilds;
  for (auto &it : itemlist)
  {
    if (it->key() == JsonDefines::TRACKTYPE && it->value() == JsonDefines::TRACKTYPEDATA)
      return true;
  }
  return false;
}
