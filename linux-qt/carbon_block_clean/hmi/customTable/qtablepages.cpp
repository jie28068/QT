...
QTablePages::QTablePages(QWidget *parent)
    : QWidget(parent), ui(new Ui::QTablePages)
{
    ui->setupUi(this);
...
void QTablePages::InitTableView()
{
    ...
    ui->tableView->setStyleSheet(Define::PAGE_TABLE_QSS);
    return;
}
...
// 查询接口
void QTablePages::SearchTableData(const QString &searchText)
{
    // 过滤数据
    qDebug() << "Searching for:" << searchText;
    QList<QStringList> filteredData;
    qDebug() << "Searching 222:" << m_dataList;
    for (const QStringList &data : m_dataList)
    {
        qDebug() << "Searching in:" << data;
    ...
    // 更新表格数据
    qDebug() << "Search results:" << filteredData.size();
    UpdateTableViewWithSearchResults(filteredData);
    emit SigSearchResults(filteredData);
}

// 根据查询结果更新表格显示
void QTablePages::UpdateTableViewWithSearchResults(const QList<QStringList> &filteredData)
{
    // 清空当前表格数据
    m_dataModel->removeRows(0, m_dataModel->rowCount());
    ...
    // 设置新的数据
    int row = 0;
    for (const QStringList &data : filteredData)
    {
        for (int col = 0; col < data.count(); ++col)
        {
            m_dataModel->setItem(row, col, new QStandardItem(data.at(col)));
            m_dataModel->item(row, col)->setTextAlignment(Qt::AlignCenter);
        }
        ++row;
    }
    ...
    // 更新分页导航栏
    m_pageNavBar->ResetNavBar();
    m_pageNavBar->InitNavbar(filteredData.size(), m_pageLines);
}